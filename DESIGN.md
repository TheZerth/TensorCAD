...
- **Field products / discrete cup product — PLANNED dedicated phase ("DEC product structure"), excluded from L9.** Products of cochains (where does edge-value × edge-value live? → the cup product) gate everything nonlinear the user's program needs: energy density `F∧⋆F` (an L10 diagnostic), every drafted Lagrangian (`Q²`, coupling terms), non-abelian `F = dA + A∧A`. Known landmine, recorded: the discrete cup product provably cannot be simultaneously associative, graded-commutative, and Leibniz-exact — the phase must *choose* which properties are exact. Slot: when energy/Lagrangian work first demands it (≈ alongside L10 diagnostics). **Accommodation NOW:** the blackboard node hierarchy is **open and n-ary** (nodes are dispatch-extensible types, each implementing its own (grade, residence) inference; never a closed unary-operator enum), so a product node — and `∇`, also deferred from L9 — arrive purely additively.
...
*End Rev 2.1. Amend deliberately; this file is the tie-breaker.*