...
# ─────────────────────────────────────────────────────────────────────────────
@testset "GASmith cross-check (conditional)" begin
    # This test block loads GASmith reference products from JSON fixtures.
    # To populate fixtures, run the GASmith export script and save the output to
    # test/fixtures/gasmith_Cl300.json  (see test/fixtures/README.md).
    #
    # The test is silently skipped if the fixture file is absent so CI passes
    # before the user generates fixtures.

    fixture_path = joinpath(@__DIR__, "fixtures", "gasmith_Cl300.json")

    if isfile(fixture_path)
        # JSON package required; add it to [extras] + [targets] in Project.toml.
        # To run: Pkg.add("JSON") in the Tensorsmith test environment, then rerun.
        @testset "GASmith Cl(3,0,0) product table" begin
            using JSON
            data = JSON.parsefile(fixture_path)
            # Expected format — see test/fixtures/README.md:
            # { "algebra": "Cl(3,0,0)", "n": 3,
            #   "products": [ {"a": [i,...], "b": [j,...], "result": [[idx, coef], ...]} ] }

            g_fix = signature_metric(
                VectorSpace(data["n"]),
                R,
                data["signature"]["p"],
                data["signature"]["q"],
            ...
            for entry in data["products"]
                idx_a  = Vector{Int}(entry["a"])
                idx_b  = Vector{Int}(entry["b"])
                a_elem = clifford_basis_element(g_fix, idx_a)
                b_elem = clifford_basis_element(g_fix, idx_b)
                prod   = a_elem * b_elem

                expected_terms = Dict{Vector{Int}, R}()
                for (idx_raw, coef_raw) in entry["result"]
                    idx = Vector{Int}(idx_raw)
                    c   = R(coef_raw)
                    iszero(c) || (expected_terms[idx] = c)
                end
                expected = CliffordTensor{R}(g_fix, expected_terms)

                @test prod == expected
            end
        end
    else
        @info "GASmith fixtures not found at $fixture_path — cross-check skipped." *
              "\nSee test/fixtures/README.md to generate them."
    end

end  # GASmith cross-check
